...
class Line {
  ...
  static unselect(state, layerID, lineID) {
    ...
    if (line) {
      state = Layer.unselect(state, layerID, 'vertices', line.vertices.get(0))
        .updatedState;
      state = Layer.unselect(state, layerID, 'vertices', line.vertices.get(1))
        .updatedState;
    ...
    state = state
      ...
      .reduce((reducedState, line) => {
        ...
        if (intersection.type === 'colinear') {
          ...
          reducedState = this.remove(reducedState, layerID, line.id)
            .updatedState;
  ...
  static endDraggingLine(state, x, y) {
    ...
    state = Layer.mergeEqualsVertices(state, layerID, line.vertices.get(0))
      .updatedState;
    state = Layer.mergeEqualsVertices(state, layerID, line.vertices.get(1))
      .updatedState;
